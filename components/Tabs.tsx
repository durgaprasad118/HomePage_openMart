...
const TabComponent: React.FC = () => {
    ...
    return (
        <div className="bg-[#F2F9FF] rounded-[28px] md:p-18 p-4 flex flex-col justify-center md:gap-y-4 gap-y-2 items-center">
            <div>
                {tabs.map((tab) => (
                    <div
                        key={tab.id}
                    ...
                    >
                        ...
                        <p className="font-semibold text-lg md:text-xl my-4">
                            - {tab.by}. {" "}
                            <span className="text-gray-400">{tab.role}</span>
                        </p>{' '}
                    </div>
                ))}
            </div>
